...
using mpv_set_option_string_fn = int (*)(mpv_handle *, const char *,
                                         const char *);
using mpv_set_property_string_fn = int (*)(mpv_handle *, const char *,
                                           const char *);
using mpv_get_property_fn = int (*)(mpv_handle *, const char *, int, void *);
using mpv_command_fn = int (*)(mpv_handle *, const char **);
using mpv_render_context_create_fn = int (*)(mpv_render_context **,
  ...
  bool load() {
    ...
    if (library_ == nullptr) {
      __android_log_print(ANDROID_LOG_ERROR, TAG,
                          "dlopen(libmpv.so) failed: %s", dlerror());
      return false;
    }

    return symbol(create, "mpv_create") &&
           symbol(initialize, "mpv_initialize") &&
           (symbol(terminate_destroy, "mpv_terminate_destroy", false) ||
            symbol(destroy, "mpv_destroy")) &&
...
private:
  ...
  bool symbol(T &target, const char *name, bool required = true) {
    ...
    if (target == nullptr && required) {
      __android_log_print(ANDROID_LOG_ERROR, TAG, "Missing libmpv symbol %s",
                          name);
      return false;
    }
...
public:
  ThumbFastEngine() {
    ...
    // Mirror thumbfast.lua's architecture: this is a dedicated, persistent,
    // paused decoder. It owns no audio, subtitles, visible Android Surface,
    // hardware decoder, or main-player state.
    setOption("config", "no");
    setOption("vo", "libmpv");
    setOption("pause", "yes");
    ...
    if (!render_ready_) {
      __android_log_print(ANDROID_LOG_ERROR, TAG,
                          "software render context creation failed");
      stop_.store(true);
  ...
  ~ThumbFastEngine() { shutdown(); }

  bool valid() const { return valid_.load(); }

  void request(std::string source, std::string user_agent,
               std::string http_headers, double position, int source_epoch) {
    if (!valid())
      return;
  ...
  jintArray waitForFrame(JNIEnv *env, int after_serial, int timeout_ms) {
    ...
    const bool ready = frame_cv_.wait_for(
        ...
        [this, after_serial] {
          return stop_.load() || frame_serial_ > after_serial;
        });

    if (!ready || stop_.load() || frame_serial_ <= after_serial ||
        frame_pixels_.empty()) {
      lock.unlock();
      release_waiter();
      return nullptr;
    ...
    if (result != nullptr) {
      ...
      if (!pixels.empty()) {
        env->SetIntArrayRegion(result, 4, static_cast<jsize>(pixels.size()),
                               pixels.data());
      }
    }
  ...
  void shutdown() {
    ...
    // A Kotlin frame poll can still be unwinding from nativeWaitForFrame().
    // Keep the object alive until it leaves the method so no JNI caller can
    // observe a freed engine.
    {
      std::unique_lock<std::mutex> lock(waiter_mutex_);
      waiter_cv_.wait_for(lock, std::chrono::seconds(1),
                          [this] { return external_waiters_ == 0; });
    }
...
private:
  void setOption(const char *name, const char *value) {
    if (mpv_ != nullptr)
      api_.set_option_string(mpv_, name, value);
  }
  ...
  bool requestIsCurrent(const Request &request) const {
    return !stop_.load() && active_requested_.load() &&
           current_generation_.load() == request.generation;
  }
  ...
  bool loadSource(const Request &request) {
    if (request.source.empty() || mpv_ == nullptr)
      return false;

    // A newer scrub position must not abort media initialization. Only
    // clear/teardown or a genuine source change invalidates an in-flight load,
    // so rapid pointer events cannot leave a half-open decoder session behind.
    if (loaded_source_epoch_ == request.source_epoch &&
        loaded_source_ == request.source && source_ready_) {
      return true;
    }
    ...
    if (loaded_source_epoch_ != request.source_epoch ||
      ...
      api_.set_property_string(mpv_, "http-header-fields",
                               request.http_headers.c_str());

      const char *args[] = {"loadfile", request.source.c_str(), "replace",
                            nullptr};
      if (api_.command(mpv_, args) < 0) {
        __android_log_print(ANDROID_LOG_WARN, TAG,
                            "loadfile failed for preview source");
        source_ready_ = false;
        return false;
      }
    ...
    const auto deadline =
        std::chrono::steady_clock::now() + SOURCE_READY_TIMEOUT;
    while (sourceIsCurrent(request) &&
           std::chrono::steady_clock::now() < deadline) {
      int64_t width = 0;
      int64_t height = 0;
      const bool has_video =
          api_.get_property(mpv_, "video-params/w", MPV_FORMAT_INT64, &width) >=
              0 &&
          width > 0 &&
          api_.get_property(mpv_, "video-params/h", MPV_FORMAT_INT64,
                            &height) >= 0 &&
          height > 0;
      if (has_video) {
        source_ready_ = true;
  ...
  bool waitForSeek(const Request &request) {
    ...
    while (requestIsCurrent(request) &&
           std::chrono::steady_clock::now() < deadline) {
      int seeking = 0;
      if (api_.get_property(mpv_, "seeking", MPV_FORMAT_FLAG, &seeking) >= 0) {
        if (seeking != 0) {
          ...
          // A nearby keyframe seek can finish before the first poll observes
          // `seeking`.
          std::this_thread::sleep_for(std::chrono::milliseconds(6));
          int seeking_again = 0;
          if (api_.get_property(mpv_, "seeking", MPV_FORMAT_FLAG,
                                &seeking_again) >= 0 &&
              seeking_again == 0) {
            return true;
          }
  ...
  bool seek(const Request &request, bool exact) {
    if (mpv_ == nullptr || !requestIsCurrent(request))
      return false;
  ...
  void scheduleRender(const Request &request) {
    // Normal libmpv property access belongs on the worker thread. The render
    // thread must stay inside mpv_render_* calls while it owns the render
    // context.
  ...
  bool waitForFirstRender(const Request &request) {
    const auto deadline =
        std::chrono::steady_clock::now() + FRAME_READY_TIMEOUT;
    std::unique_lock<std::mutex> lock(request_mutex_);
    while (requestIsCurrent(request) &&
           rendered_generation_.load() < request.generation &&
           std::chrono::steady_clock::now() < deadline) {
    ...
    while (!stop_.load()) {
      Request request;
      {
        ...
        request_cv_.wait(lock, [this] {
          return stop_.load() || (active_ && latest_request_.generation >
                                                 processed_generation_);
        });
        if (stop_.load())
          break;
      ...
      if (last_fast_seek != std::chrono::steady_clock::time_point::min() &&
          now < due) {
        std::unique_lock<std::mutex> lock(request_mutex_);
      ...
      // While the finger moves, use cheap keyframe seeks and let
      // newest-target-wins conflation discard intermediate positions that the
      // decoder could never display in time anyway.
      if (!seek(request, false) || !requestIsCurrent(request))
        continue;
      ...
      // Once input settles, refine only the secondary core to the exact
      // requested timestamp.
      if (!waitForSettleOrNewRequest(request) || !requestIsCurrent(request))
        continue;
      if (!seek(request, true) || !requestIsCurrent(request))
        continue;
      scheduleRender(request);
    }
  }
  ...
  std::pair<int, int> previewSize() {
    double aspect = 0.0;
    if (api_.get_property(mpv_, "video-params/aspect", MPV_FORMAT_DOUBLE,
                          &aspect) < 0 ||
        !std::isfinite(aspect) || aspect <= 0.0) {
      int64_t width = 0;
      int64_t height = 0;
      if (api_.get_property(mpv_, "video-params/w", MPV_FORMAT_INT64, &width) >=
              0 &&
          width > 0 &&
          api_.get_property(mpv_, "video-params/h", MPV_FORMAT_INT64,
                            &height) >= 0 &&
  ...
  void renderLoop() {
    ...
    const int create_result =
        api_.render_context_create(&render_context, mpv_, create_params);

    // One maximum-size aligned surface is reused for every frame. Allocate it
    // before publishing render_ready_ so construction cannot succeed with a
    // dead render thread.
    const size_t stride =
        static_cast<size_t>((PREVIEW_MAX_EDGE * 4 + 63) & ~63);
    void *aligned_memory = nullptr;
    const bool buffer_ready =
        create_result >= 0 && render_context != nullptr &&
        posix_memalign(&aligned_memory, 64, stride * PREVIEW_MAX_EDGE) == 0 &&
    ...
    while (!stop_.load()) {
      RenderRequest request;
      {
        ...
        render_cv_.wait(lock,
                        [this] { return stop_.load() || has_pending_render_; });
        if (stop_.load())
          break;
      ...
      const auto deadline =
          std::chrono::steady_clock::now() + FRAME_READY_TIMEOUT;
      while (!stop_.load() && active_requested_.load() &&
             current_generation_.load() == request.generation &&
             current_source_epoch_.load() == request.source_epoch &&
             std::chrono::steady_clock::now() < deadline) {
        if ((api_.render_context_update(render_context) &
      ...
      if (!frame_ready || !active_requested_.load() ||
          current_generation_.load() != request.generation ||
          current_source_epoch_.load() != request.source_epoch) {
        continue;
      }
      ...
      for (int y = 0; y < height; ++y) {
        ...
        for (int x = 0; x < width; ++x) {
          ...
          pixels[static_cast<size_t>(y * width + x)] = static_cast<jint>(
              0xFF000000u | (static_cast<uint32_t>(r) << 16) |
              (static_cast<uint32_t>(g) << 8) | static_cast<uint32_t>(b));
        }
      }

      // Serialize final publication with request generation changes. A newer
      // pointer target can arrive while BGR0 is being converted; an older frame
      // must never become visible after it.
      {
...
Java_app_gyrolet_mpvrx_ui_player_thumbfast_NativeThumbFast_nativeCreate0(
    JNIEnv *, jobject) {
...
Java_app_gyrolet_mpvrx_ui_player_thumbfast_NativeThumbFast_nativeRequest0(
    JNIEnv *env, jobject, jlong handle, jstring source, jstring user_agent,
    jstring http_headers, jdouble position, jint source_epoch) {
  ThumbFastEngine *engine = fromHandle(handle);
  if (engine == nullptr)
    return;
  engine->request(fromJString(env, source), fromJString(env, user_agent),
                  fromJString(env, http_headers), static_cast<double>(position),
...
Java_app_gyrolet_mpvrx_ui_player_thumbfast_NativeThumbFast_nativeWaitForFrame0(
    JNIEnv *env, jobject, jlong handle, jint after_serial, jint timeout_ms) {
  ThumbFastEngine *engine = fromHandle(handle);
  if (engine == nullptr)
    return nullptr;
