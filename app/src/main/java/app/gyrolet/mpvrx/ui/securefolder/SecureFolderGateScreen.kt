 ...
 * normal entry (PIN + eye toggle to reveal it), and the forgot-PIN recovery flow
 * (security question -> new PIN).
 *
      ...
      Box(
        ...
        AnimatedContent(
          ...
          when (step) {
            ...
            SecureFolderViewModel.GateStep.SETUP_PIN ->
              ChoosePinContent(
                title = "Set a PIN",
                subtitle = "You'll use this to open your Secure Folder.",
                error = gateError,
                onSubmit = { pin -> viewModel.submitNewPin(pin) },
              )

            SecureFolderViewModel.GateStep.SETUP_SECURITY_QUESTION ->
              SecurityQuestionSetupContent(
                error = gateError,
                onSubmit = { question, answer -> viewModel.submitSecurityQuestion(question, answer) },
              )
            ...
            SecureFolderViewModel.GateStep.FORGOT_PIN_NEW_PIN ->
              ChoosePinContent(
                ...
                error = gateError,
                onSubmit = { pin ->
                  viewModel.submitNewPin(pin)
                  viewModel.confirmPendingPinOnly()
                },
              )
          }
        }
...
@Composable
private fun SecurityQuestionSetupContent(
  error: String?,
  ...
  Column(
    ...
    Text(
      "Used to recover access if you forget your PIN.",
      style = MaterialTheme.typography.bodyMedium,
      color = MaterialTheme.colorScheme.onSurfaceVariant,
      textAlign = TextAlign.Center,
      modifier = Modifier.padding(top = 4.dp, bottom = 24.dp),
    ...
    OutlinedTextField(
      ...
      label = { Text("Question") },
      placeholder = { Text("e.g. What was your first pet's name?") },
      singleLine = true,
    ...
    Button(
      onClick = { onSubmit(question.trim(), answer) },
